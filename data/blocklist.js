...
export const HARD_MESSAGE_PATTERNS = [
  ...
  rule("depo-wallet", /\b(?:depo|deposit|mindep|dp)\s*(?:via\s*)?(?:qris|dana|ovo|gopay|pulsa|linkaja|shopeepay)\b/i, 6),
  rule("depo-amount", /\b(?:depo|deposit|mindep|dp)\s*(?:(?:minimal|min)\s+)?(?:5k|10k|20k|25k|50k|100k|\d{1,3}\s?(?:k|rb))\b/i, 5),
  rule("slot-claim", /\b(?:slot|togel|toto|casino|judi)\s+(?:gacor|online|resmi|terpercaya|terbaik|hari\s*ini)\b/i, 6),
  ...
  rule("bonus-member", /\bbonus\s+(?:member\s*baru|new\s*member|deposit|cashback|rollingan|rebate|turnover)\b/i, 5),
  rule("slot-pattern", /\bpola\s+(?:slot|gacor|scatter|rtp|maxwin|jp)\b/i, 5),
  rule("scatter-claim", /\bscatter\s+(?:hitam|merah|emas|gacor|maxwin)\b/i, 5),
  ...
  rule("wd-fast", /\b(?:wd|withdraw|wede)(?:nya)?\s+(?:lancar|cepat|instan|mudah|kilat|cair)\b/i, 5),
...
export const PROMO_MESSAGE_PATTERNS = [
  rule("instant-cashout", /\bcair\s+(?:instan|kilat|terus|setiap\s+hari)\b/i, 3),
  ...
  rule("jp-claim", /\b(?:jp|maxwin|jackpot)\s+(?:besar|gede|terus|parah|gampang|banyak)\b/i, 4),
