...
function isEmojiRange(cp) {
  return (cp >= 0x2600 && cp <= 0x27bf) || (cp >= 0x1f300 && cp <= 0x1faff) || (cp >= 0x1f1e0 && cp <= 0x1f1ff) || (cp >= 0xfe00 && cp <= 0xfe0f);
}
...
const linesOut = ["// AUTO-GENERATED from scripts/charset.json by scripts/build-homoglyphs.js.", "// Do not hand-edit — regenerate with `bun scripts/build-homoglyphs.js`.", "", "// Map<codepoint, string>. Single-char value = unambiguous fold.", "// Multi-char value = ambiguous — normalizeText branches and emits a variant", "// per interpretation.", "export const HOMOGLYPHS = new Map(["];
