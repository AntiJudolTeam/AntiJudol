...
// cuimp speaks an inferior log API; map only its warnings/errors into our logger.
const cuimpLogger = {
  debug: () => {},
  info: () => {},
...
export async function fetchViaImpersonate(url, { method = "GET", headers = {}, body = null } = {}) {
  ...
  logger.debug("impersonate", method, url, `status=${res.status}`, `bytes=${rawBody?.length ?? 0}`);
