...
describe("normalizeText: basic behavior", () => {
  ...
  test('undefined returns [""]', () => expect(normalizeText(undefined)).toContain(""));
  test('number returns [""]', () => expect(normalizeText(123)).toContain(""));
  test('object returns [""]', () => expect(normalizeText({})).toContain(""));
  test("plain ASCII passthrough", () => expect(normalizeText("hello world")).toContain("hello world"));
  test("mixed case lowercased", () => expect(normalizeText("Hello World")).toContain("hello world"));
  test("trims outer whitespace", () => expect(normalizeText("  hi  ")).toContain("hi"));
  test("collapses inner whitespace", () => expect(normalizeText("a  b   c")).toContain("a b c"));
  test("tabs and newlines collapse to single space", () => expect(normalizeText("a\tb\nc\r\nd")).toContain("a b c d"));
  test("only whitespace returns empty", () => expect(normalizeText("   \t\n  ")).toContain(""));
  test("digits preserved", () => expect(normalizeText("order 1234")).toContain("order 1234"));
});

describe("normalizeText: fullwidth Latin", () => {
  test("fullwidth uppercase Ａ–Ｚ", () => expect(normalizeText(range26(0xff21))).toContain(LOWER));
  test("fullwidth lowercase ａ–ｚ", () => expect(normalizeText(range26(0xff41))).toContain(LOWER));
  test("fullwidth digits normalize", () => expect(normalizeText("０１２３４５６７８９")).toContain("0123456789"));
  test("fullwidth mixed sentence", () => expect(normalizeText("ＨＥＬＬＯ Ｗｏｒｌｄ")).toContain("hello world"));
});
...
describe("normalizeText: Mathematical alphanumeric alphabets", () => {
  const cases = [
    ...
    ["fraktur uppercase 𝔄–𝔜", 0x1d504, { 2: 0x212d, 7: 0x210c, 8: 0x2111, 17: 0x211c, 25: 0x2128 }],
...
describe("normalizeText: Enclosed / circled / squared letters", () => {
  test("circled uppercase Ⓐ–Ⓩ", () => expect(normalizeText(range26(0x24b6))).toContain(LOWER));
  test("circled lowercase ⓐ–ⓩ", () => expect(normalizeText(range26(0x24d0))).toContain(LOWER));
  test("squared 🄰–🅉", () => expect(normalizeText(range26(0x1f130))).toContain(LOWER));
  test("negative circled 🅐–🅩", () => expect(normalizeText(range26(0x1f150))).toContain(LOWER));
...
describe("normalizeText: diacritics stripped", () => {
  ...
  test("precomposed À–Å all flatten to a (then collapse)", () => expect(normalizeText("ÀÁÂÃÄÅ")).toContain("aa"));
  test("varied diacritics on different letters", () => expect(normalizeText("ÀÉÎÕÜ")).toContain("aeiou"));
  test("decomposed a + combining acute", () => expect(normalizeText("á")).toContain("a"));
  test("stacked combining marks", () => expect(normalizeText("á̂̃")).toContain("a"));
  test("cedilla", () => expect(normalizeText("façade")).toContain("facade"));
  test("tilde n", () => expect(normalizeText("piñata")).toContain("pinata"));
  test("zalgo text (many combining marks)", () => expect(normalizeText("h́̂ẽ̄l̅̆lo")).toContain("hello"));
});
...
describe("normalizeText: superscripts and subscripts", () => {
  ...
  test("superscript letters ⁿⁱ fold to n i", () => expect(normalizeText("log ⁿ values ⁱ")).toContain("log n values i"));
  test("subscript digit ₂ folds to 2", () => expect(normalizeText("H₂O")).toContain("h2o"));
  test("subscript letter ₐ folds to a", () => expect(normalizeText("Cₐ test")).toContain("ca test"));
});

describe("normalizeText: emojis removed", () => {
  test("single emoji", () => expect(normalizeText("hi 👋")).toContain("hi"));
  test("emoji with skin tone modifier", () => expect(normalizeText("👍🏽 ok")).toContain("ok"));
  test("ZWJ family sequence", () => expect(normalizeText("👨‍👩‍👧 family")).toContain("family"));
  test("flag emoji (regional indicators) fold to ASCII letters via confusables", () =>
    expect(normalizeText("hello 🇮🇩")).toContain("hello id"));
  test("geometric colored circles", () => expect(normalizeText("🟢🟡🔴 go")).toContain("go"));
  test("misc symbols & dingbats", () => expect(normalizeText("★ ✓ ☂ rain")).toContain("rain"));
  test("emoji between letters removed without adding space", () => expect(normalizeText("ab🎉cd")).toContain("abcd"));
});

describe("normalizeText: zero-width / invisible chars", () => {
  test("zero-width space (U+200B)", () => expect(normalizeText("ab​cd")).toContain("abcd"));
  test("zero-width non-joiner (U+200C)", () => expect(normalizeText("ab‌cd")).toContain("abcd"));
  test("zero-width joiner (U+200D)", () => expect(normalizeText("ab‍cd")).toContain("abcd"));
  test("LTR/RTL marks (U+200E/U+200F)", () => expect(normalizeText("ab‎cd‏ef")).toContain("abcdef"));
  test("LTR/RTL embedding/override (U+202A–U+202E)", () => expect(normalizeText("‮abc‬")).toContain("abc"));
  test("word joiner (U+2060)", () => expect(normalizeText("ab⁠cd")).toContain("abcd"));
  test("byte-order mark (U+FEFF)", () => expect(normalizeText("﻿abc")).toContain("abc"));
  test("soft hyphen (U+00AD)", () => expect(normalizeText("ab­cd")).toContain("abcd"));
});
...
describe("normalizeText: homoglyph folding", () => {
  ...
  test("Cyrillic homoglyph word (АВСЕНКМОРТХ)", () => expect(normalizeText("АВСЕНКМОРТХ")).toContain("abcehkmoptx"));
  test("Greek homoglyphs (ΑΒΕΖΗΙΚΜΝΟΡΤΥΧ)", () => expect(normalizeText("ΑΒΕΖΗΙΚΜΝΟΡΤΥΧ")).toContain("abezhlkmnoptyx"));
  test("CJK 'kantorbola' variant 1", () => expect(normalizeText("Ҝ卂几ㄒㄖ尺乃ㄖㄥ卂")).toContain("kantorbola"));
...
describe("normalizeText: realistic mixed attacks", () => {
  ...
  test("real-world judol spam with script-l and curly quotes", () => {
    const spam = "Kantorboℓa99.art Permainan Yang Lagi Bagi Pro’fits “STARLIGHT PRINCES” RTP 99% Gas Daf’tar Sekarang";
...
describe("normalizeText: homoglyph data file coverage", () => {
  ...
  const UNSUPPORTED = new Set(["emojiCharMap", "uniqSymbolsFontCharMap", "bigBlockCharMap", "s2", "s4"]);

  for (const [mapName, m] of Object.entries(maps)) {
    if (typeof m !== "object" || m === null) continue;
    if (UNSUPPORTED.has(mapName)) continue;

    const stringified = [...UPPER].map((ch) => m[ch] ?? ch).join("") + [...LOWER].map((ch) => m[ch] ?? ch).join("");
