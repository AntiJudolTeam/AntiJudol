const PATTERNS = [/ crossorigin="[^"]*"/g, / integrity="[^"]*"/g, /<script[^>]*src="[^"]*cdn-cgi[^"]*"[^>]*>[\s\S]*?<\/script>/gi, /<script[^>]*rocket-loader[^>]*>[\s\S]*?<\/script>/gi, /<script[^>]*cloudflareinsights[^>]*>[\s\S]*?<\/script>/gi, /<link[^>]*builds\/meta[^>]*>/gi];
