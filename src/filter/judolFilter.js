...
const OBFUSCATED_CHAIN_RE = /\b[a-z0-9](?:\s*[._\-\/\\,:;*+|~]\s*[a-z0-9]){1,}\b/gi;
...
function tightenIntraWord(text) {
  const punctTight = text.replace(OBFUSCATED_CHAIN_RE, (m) => m.replace(/[\s._\-\/\\,:;*+|~]+/g, ""));

  const spaceTight = punctTight.replace(SPACED_CHAIN_RE, (m) => m.replace(/\s+/g, ""));

  return spaceTight.replace(MULTI_CHAR_CHAIN_RE, (m) => m.replace(/[\s._\-]+/g, ""));
}
...
export function decide(donator, message) {
  ...
  const messageEvidenceBuckets = Number(ctx.msg.highBrands.length > 0) + Number(ctx.msg.ambiguousBrands.length > 0) + Number(ctx.msg.promo.length > 0) + Number(ctx.msg.weak.length >= 2) + Number(ctx.msg.suspicious.length > 0);
