...
function foldLeet(text, map) {
  return text.replace(LEET_CHAR_RE, (c) => map[c] ?? c);
}
...
export function matchToken(raw, opts = {}) {
  const { wordlist = loadWordlist(), index = buildWordlistIndex(wordlist), sensitive = SENSITIVE_TERMS, enableDictionaryFuzzy = true } = opts;
